 ...
 * @param {Object} tagOptions Tag config options.
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @throws {Error} if the tag creation failed.
 */
export async function tag(tagName, ref, { tagAnnotate, tagSign, tagMessage }, execaOptions) {
  const args = ["tag", tagName, ref];
  const gitTagSign = await shouldSignTag(execaOptions);
  if (gitTagSign || tagSign) {
    args.push("-s", "-m", tagMessage);
  } else if (tagAnnotate) {
    args.push("-a", "-m", tagMessage);
  }
...
/**
 * Get whether the tag should be gpg signed.
 *
 * @param {Object} [execaOpts] Options to pass to `execa`.
 *
 * @return {Boolean}
 */
async function shouldSignTag(execaOptions) {
  try {
    return (await execa("git", ["config", "--get", "tag.gpgsign"], execaOptions)).stdout === "true";
  } catch (error) {
    debug(error);
  }
}
